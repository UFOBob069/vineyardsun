...
async function printfulRequest(path: string, init?: RequestInit) {
  const { token, storeId } = printfulConfig();
...
async function findExistingOrder(externalId: string) {
  const encodedId = encodeURIComponent(`@${externalId}`);
  const { response, body } = await printfulRequest(`/orders/${encodedId}`);
...
async function confirmOrder(externalId: string) {
  const encodedId = encodeURIComponent(`@${externalId}`);
...
export async function submitPrintfulOrder({
  ...
  const externalId = `vs-${orderId}`;
  const existing = await findExistingOrder(externalId);
  if (existing) {
    ...
    const confirmed = await confirmOrder(externalId);
  ...
  const payload = {
    ...
    items: lines.map((line) => ({
      ...
      external_variant_id: String(line.variantId),
  ...
  if (!response.ok || !body.result) {
    ...
    // another one.
    const racedOrder = await findExistingOrder(externalId);
